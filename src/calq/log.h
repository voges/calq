...
#define LOG_FATAL(x)                                    \
    {                                                   \
        calq::Log &log = calq::Log::instance();         \
        log.out(x, calq::Log::Level::FATAL, std::cerr); \
    }
#define LOG_ERROR(x)                                    \
    {                                                   \
        calq::Log &log = calq::Log::instance();         \
        log.out(x, calq::Log::Level::ERROR, std::cerr); \
    }
#define LOG_WARN(x)                                    \
    {                                                  \
        calq::Log &log = calq::Log::instance();        \
        log.out(x, calq::Log::Level::WARN, std::cerr); \
    }
#define LOG_INFO(x)                                    \
    {                                                  \
        calq::Log &log = calq::Log::instance();        \
        log.out(x, calq::Log::Level::INFO, std::cout); \
    }
#define LOG_DEBUG(x)                                    \
    {                                                   \
        calq::Log &log = calq::Log::instance();         \
        log.out(x, calq::Log::Level::DEBUG, std::cout); \
    }
#define LOG_TRACE(x)                                    \
    {                                                   \
        calq::Log &log = calq::Log::instance();         \
        log.out(x, calq::Log::Level::TRACE, std::cout); \
    }
