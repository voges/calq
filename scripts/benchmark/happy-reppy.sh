...
${python} ${hap_py} --verbose --threads ${num_threads} ${golden_vcf_gz} ${variants_vcf} -f ${golden_bed} -o ${variants_vcf}.happy -r ${ref_fasta} --roc VQLSOD &>>${log_txt}
...
for i in "${variants_vcf}".happy.roc.Locations.*; do echo "$i,"; done >> "${variants_vcf}".happy.rep.tsv
