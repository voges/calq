...
static int encode(const calqapp::ProgramOptions& programOptions) {
    ...
    calqapp::SAMFileHandler sH(programOptions.inputFilePath,
                               programOptions.referenceFilePath);

    calqapp::CQFile file(programOptions.outputFilePath,
                         calqapp::File::Mode::MODE_WRITE);
    file.writeHeader(programOptions.blockSize);

    gabac::EncodingConfiguration config;
    calqapp::File configurationFile("config.json",
    ...
    while (sH.readBlock(programOptions.blockSize) != 0) {
        ...
        file.writeBlock(programOptions.options, config, decBlock, encSide,
                        unmappedString, programOptions.debugStreams,
        ...
        uncompressedMappedQualSize = std::accumulate(
            encBlock.qvalues.begin(), encBlock.qvalues.end(),
    ...
    auto diffTimeMs =
        std::chrono::duration_cast<std::chrono::milliseconds>(diffTime).count();
    auto diffTimeS =
    ...
    CALQ_LOG("  Took %d ms ~= %d s ~= %d m ~= %d h", (int)diffTimeMs,
             (int)diffTimeS, (int)diffTimeM, (int)diffTimeH);
    ...
    CALQ_LOG("  Compression ratio: %4.2f%%",
             (double)file.nrWrittenBytes() * 100 /
...
static int decode(const calqapp::ProgramOptions& po) {
    ...
    calqapp::CQFile file(programOptions.inputFilePath,
                         calqapp::File::Mode::MODE_READ);

    calqapp::File qualFile(programOptions.outputFilePath,
                           calqapp::File::Mode::MODE_WRITE);

    file.readHeader(&programOptions.blockSize);

    calqapp::SAMFileHandler sH(programOptions.sideInformationFilePath,
                               programOptions.referenceFilePath);
        ...
        for (const auto& b : unmappedInfo.mappedFlags) {
            if (b) {
                ...
                std::string read = unmappedValues.substr(
                    unmappedPos, unmappedSideIt->length());
