...
void File::open(const std::string &path, Mode mode) {
    ...
    auto m =
        (mode_ == Mode::MODE_READ) ? std::ifstream::in : std::ifstream::out;
    try {
    ...
    try {
        ...
        throwErrorException(std::string("Error obtaining file size: ") +
                            e.what());
    }
}
...
void File::close() {
    if (filestream.is_open()) {
        try {
            ...
            throwErrorException(std::string("Failed to close file: ") +
                                e.what());
        }
    }
}
...
size_t File::readByte(unsigned char *byte) { return readValue(byte, 1); }
...
size_t File::readUint16(uint16_t *word) { return readValue(word, 1); }
...
size_t File::writeByte(unsigned char byte) { return writeValue(&byte, 1); }
...
size_t File::writeUint16(uint16_t word) { return writeValue(&word, 1); }
