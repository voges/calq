...
TEST(Quantizer, EmptyLut) {  // NOLINT(cert-err58-cpp)
    ...
    // There is no table in the quantizer. Thus we expect these functions to throw exceptions.
...
TEST(Quantizer, PopulatedLut) {  // NOLINT(cert-err58-cpp)
    ...
    // The function valueToIndex() performs a look-up in the LUT; this is however not initialized within the Quantizer.
