...
### Sort BAM file
$samtools sort -T $samtoolsTmpDir -@ $num_threads -O bam $root.aln_bwa.fixmate.bam > $root.aln_bwa.fixmate.sorted.bam
rm -f $root.aln_bwa.fixmate.bam

### Add read group name
java -jar -Djava.io.tmpdir=$javaIOTmpDir $picard_jar AddOrReplaceReadGroups I=$root.aln_bwa.fixmate.sorted.bam O=$root.aln_bwa.fixmate.sorted.rg.bam RGID=1 RGLB=lib1 RGPL=illumina RGPU=unit1 RGSM=$sample
...
java -Xmx2g -jar -Djava.io.tmpdir=$javaIOTmpDir $picard_jar MarkDuplicates VALIDATION_STRINGENCY=LENIENT I=$root.aln_bwa.fixmate.sorted.rg.realn.bam O=$root.aln_bwa.fixmate.sorted.rg.realn.dupmark.bam M=$root.dedup_metrics.txt
