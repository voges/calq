...
java -jar -Djava.io.tmpdir=$javaIOTmpDir $picard_jar SortSam I=$root.aln_bwa.sam O=$root.aln_bwa.sorted.bam SORT_ORDER=coordinate
rm -f $root.aln_bwa.sam

### Mark duplicates in the BAM file
java -jar -Djava.io.tmpdir=$javaIOTmpDir $picard_jar MarkDuplicates I=$root.aln_bwa.sorted.bam O=$root.aln_bwa.sorted.dupmark.bam M=$root.dedup_metrics.txt ASSUME_SORTED=true
rm -f $root.dedup_metrics.txt
rm -f $root.aln_bwa.sorted.bam

### Add read group name
java -jar -Djava.io.tmpdir=$javaIOTmpDir $picard_jar AddOrReplaceReadGroups I=$root.aln_bwa.sorted.dupmark.bam O=$root.aln_bwa.sorted.dupmark.rg.bam RGID=1 RGLB=lib1 RGPL=illumina RGPU=unit1 RGSM=$sample
rm -f $root.aln_bwa.sorted.dupmark.bam

### Index the BAM file
java -jar -Djava.io.tmpdir=$javaIOTmpDir $picard_jar BuildBamIndex I=$root.aln_bwa.sorted.dupmark.rg.bam

###############################################################################
#                      Recalibrate base quality scores                        #
###############################################################################
#java -jar -Djava.io.tmpdir=$javaIOTmpDir $GenomeAnalysisTK_jar -nct $num_threads -T baseRecalibrator -R $ref_fasta -I $root.aln_bwa.sorted.dupmark.rg.bam -knownSites $dbsnps_vcf -knownSites $mills_vcf -knownSites $indels_vcf -o $root.bqsr.table
#java -jar -Djava.io.tmpdir=$javaIOTmpDir $GenomeAnalysisTK_jar -nct $num_threads -T PrintReads -R $ref_fasta -I $root.aln_bwa.sorted.dupmark.rg.bam -BQSR $root.bqsr.table -o $root.aln_bwa.sorted.dupmark.rg.recal.bam
...
SEC=10
SCC=30
java -jar -Djava.io.tmpdir=$javaIOTmpDir $GenomeAnalysisTK_jar -T HaplotypeCaller -R $ref_fasta -I $root.aln_bwa.sorted.dupmark.rg.bam --genotyping_mode DISCOVERY -stand_emit_conf $SEC -stand_call_conf $SCC -o $root.raw_variants.vcf
