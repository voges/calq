...
TEST(UniformMinMaxQuantizer, Initialization) {  // NOLINT(cert-err58-cpp)
    ...
    EXPECT_THROW(calq::UniformMinMaxQuantizer q1(1, 0, 2), calq::ErrorException);  // minValue > maxValue
    EXPECT_THROW(calq::UniformMinMaxQuantizer q2(0, 1, 1), calq::ErrorException);  // numSteps <=1
...
TEST(UniformMinMaxQuantizer, PositiveRange) {  // NOLINT(cert-err58-cpp)
    ...
    EXPECT_EQ(q.indexToReconstructedValue(1), 4);
}
...
TEST(UniformMinMaxQuantizer, NegativeRange) {  // NOLINT(cert-err58-cpp)
    ...
    EXPECT_EQ(q.indexToReconstructedValue(0), -5);
    EXPECT_EQ(q.indexToReconstructedValue(1), -1);
}
...
TEST(UniformMinMaxQuantizer, EightBinning) {  // NOLINT(cert-err58-cpp)
    ...
    EXPECT_EQ(q.indexToReconstructedValue(7), 40);
}
